...
public abstract class VerificationInfo {
    public static final int CONFIRM_CODE_EXPIRE_MINURE = 15;
    public static final int CONFIRMATION_EXPIRE_MINURE = 60;
    ...
    public boolean isValidToConfirm(LocalDateTime time) {
        return time.isAfter(createdAt) && time.isBefore(createdAt.plusMinutes(CONFIRM_CODE_EXPIRE_MINURE));
    }
    ...
    public boolean isConfirmationValidAt(LocalDateTime time) {
        return time.isAfter(confirmedAt) && time.isBefore(confirmedAt.plusMinutes(CONFIRMATION_EXPIRE_MINURE));
    }
